...
void DealWithMsg(LPVOID Context)
{
	try
	{
		...
		if (isFriendMsg == TRUE) {
			swprintf_s(jsonStr, L"{'wxid':'%s','content':'%s','sztype':'%s','source':'%s','wxname':'%s','sender':'%s','senderName':'%s'}", msg->wxid, msg->content, msg->sztype, msg->source, msg->wxname, msg->sender, msg->sendername);
			OutputDebugStringW(jsonStr);
		//	OutputDebugStringW(msg->content);
			
			if (p_ws != NULL && p_ws->getReadyState() == easywsclient::WebSocket::OPEN) {
				p_ws->send(UnicodeToUtf82(jsonStr));
			}
			else
			{
				OutputDebugStringW(L"WS was closed, so ignore redirect it");
			}
		}

	
		//这里处理自动聊天 remove by nxu
